...
var App = function(makehuman, dat, _, THREE, Detector, Nanobar, Stats) {
    ...
    function App(resources, modeling_sliders) {
        ...
        //this.SCREEN_WIDTH = window.innerHeight*0.8*0.466;  //2
        //this.SCREEN_HEIGHT = window.innerHeight*0.8*0.817;

        this.SCREEN_WIDTH = window.innerWidth*0.2+17;  //2
        this.SCREEN_HEIGHT = window.innerWidth*0.2+17;
    ...
    App.prototype.init = function init() {
        ...
        self.camera.position.set(0, 15, 14);
        ...
        return new Promise(function(resolve, reject) {
            ...
            self.human.loadTargets(`${self.resources.baseUrl}targets/${self.resources.targets}`).then(() => {
                ...
                document.querySelector("#container canvas").style.opacity = 1;
    ...
    App.prototype.setHumanDefaults = function(){
        ...
        randomPose = _.sample(['standing02'])
        ...
        randomHair = _.sample(['ponytail01'])
        this.human.proxies.toggleProxy(randomHair,true)
        this.human.proxies.toggleProxy('data/proxies/eyes/Low-Poly/Low-Poly.json',true)
        this.human.proxies.toggleProxy('Eyelashes01',true)
        this.human.proxies.toggleProxy('HighPolyEyes',true)
    ...
    App.prototype.setModifierDefaults = function () {
        ...
        macroControllers.find(c=>c.property=="African").setValue(0.05)
        macroControllers.find(c=>c.property=="Asian").setValue(0.21)
        macroControllers.find(c=>c.property=="Caucasian").setValue(0.72)
        macroControllers.find(c=>c.property=="Age").setValue(0.4)
        macroControllers.find(c=>c.property=="Height").setValue(0.7)
        macroControllers.find(c=>c.property=="Weight").setValue(0.7)

        var genderControllers = this.gui.gui.__folders.Modifiers.__folders["Gender"].__folders.Breast.__controllers
        genderControllers.find(c=>c.property=="Breast size").setValue(0.64);
        genderControllers.find(c=>c.property=="Breast firmness").setValue(0.94);
        genderControllers.find(c=>c.property=="Vertical position").setValue(0.6);
    }

    App.prototype.onWindowResize = function() {

        this.camera.updateProjectionMatrix();
        this.renderer.setSize( window.innerWidth*0.2+17, window.innerWidth*0.2+17);
    }
